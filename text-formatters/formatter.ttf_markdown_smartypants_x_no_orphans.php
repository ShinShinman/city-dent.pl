<?php
	...
	// Template class name must be constructed as: formatter___[type]ttf_markdown_smartypants_x_no_orphans
	...
	// When saving, ___[type]ttf_markdown_smartypants_x_no_orphans will be replaced by class name entered in editor.
	...
	Class formatterttf_markdown_smartypants_x_no_orphans extends TextFormatter {
		...
		public function __construct() {
			...
			$this->_formatters = array (
				'ttf_wiszace_ziomy' => 'Wiszace ziomy',
		...
		public function about() {
			return array(
				'name' => 'Markdown SmartyPants x No orphans', // required
				'author' => array(
					'name' => 'Olaf Schindler',
				...
				'version' => '1.4',
				'release-date' => '2017-10-05T13:29:49+00:00',
				'description' => 'Formatting text in the following order: Wiszace ziomy → Markdown Extra (With SmartyPants)',
		...
		public function run($string) {
			...
			if (isset(self::$_recursion[__CLASS__]) && self::$_recursion[__CLASS__] > 0) {
				return "<!-- CHAIN RECURSION ERROR\n*\n*\t"
					.__("Detected text formatter recursion on %s", array("\n*\tMarkdown SmartyPants x No orphans (ttf_markdown_smartypants_x_no_orphans)"))
					."\n*\t"
			...
			FormatterChain::$stack[] = 'Markdown SmartyPants x No orphans (ttf_markdown_smartypants_x_no_orphans)';
			$result = $string;
		...
		public function ttf_form(&$form, &$page) {
			...
			foreach ($formatters as $id => $about) {
				if ($about['handle'] == 'ttf_markdown_smartypants_x_no_orphans') continue;
